...
class CacheService {
  ...
  public async clear(cacheName: string): Promise<ActionResponse> {
    let clearPromise = utils.restCall(
      this.endpoint + '/caches/' + encodeURIComponent(cacheName) + '?action=clear',
      'GET'
    );
  ...
  public async purgeIndexes(cacheName: string): Promise<ActionResponse> {
    return utils
      .restCall(
        this.endpoint + '/caches/' + encodeURIComponent(cacheName) + '/search/indexes?action=clear',
        'GET'
      )
      .then((response) => {
        if (response.ok) {
  ...
  public async reindex(cacheName: string): Promise<ActionResponse> {
    return utils
      .restCall(
        this.endpoint + '/caches/' + encodeURIComponent(cacheName) + '/search/indexes?action=mass-index&mode=async',
        'GET'
      )
      .then((response) => {
        if (response.ok) {
