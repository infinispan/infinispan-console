...
const SchemaEditPage = () => {
  ...
  const { schemaDetail, loading, error, reload } = useFetchProtobufSchemaDetailed(schemaName || '');
  ...
  const panelContent = (
    <DrawerPanelContent isResizable id="schema-info-panel" defaultSize="300px" minSize="200px">
      <DrawerHead>
      ...
      <div style={{ padding: '0 1rem 1rem 1rem' }}>
        {hasErrors ? (
          <Alert isInline isPlain variant={AlertVariant.danger} title={t('schemas.edit-page.validation-errors')}>
            <p>{schemaDetail!.error!.message}</p>
            {schemaDetail!.error!.cause && <p>{schemaDetail!.error!.cause}</p>}
          </Alert>
        ) : (
          <Alert isInline isPlain variant={AlertVariant.success} title={t('schemas.edit-page.validation-valid')} />
        )}
          ...
          <Content component="ul">
            {schemaDetail!.caches.map((cacheName) => (
              <Content component="li" key={cacheName}>
                <Link to={'/cache/' + encodeURIComponent(cacheName)}>{cacheName}</Link>
              </Content>
            ))}
          </Content>
        )}
  ...
  const displayActions = (
    ...
    >
      <DropdownList>
        ...
        <DropdownItem key="delete" onClick={() => setDeleteSchemaName(schemaName || '')}>
  ...
  return (
    <>
      ...
      />
      <PageSection className="schema-editor-section" isFilled padding={{ default: 'noPadding' }}>
      ...
      <PageSection variant="secondary" padding={{ default: 'padding' }}>
        <Toolbar>
          <ToolbarContent>
            ...
            <ToolbarItem>
              <Button variant={ButtonVariant.link} onClick={handleCancel} aria-label="cancel-edit-schema-button">
