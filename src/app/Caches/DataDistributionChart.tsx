...
const DataDistributionChart = (props: { cacheName: string }) => {
  ...
  const [statsOption, setStatsOption] = useState<string>(
    DataDistributionStatsOption.TotalEntries
  );
  ...
  const onSelectStatsOptions = (event, selection, isPlaceholder) => {
    if (
      selection === t('caches.cache-metrics.data-distribution-option-entries')
    )
      setStatsOption(DataDistributionStatsOption.TotalEntries);
    else if (
      selection === t('caches.cache-metrics.data-distribution-option-memory')
    )
    ...
    return (
      <LevelItem>
        ...
        >
          <SelectOption
            key={0}
            value={t('caches.cache-metrics.data-distribution-option-entries')}
          />
          <SelectOption
            key={1}
            value={t('caches.cache-metrics.data-distribution-option-memory')}
          />
        </Select>
      </LevelItem>
    );
