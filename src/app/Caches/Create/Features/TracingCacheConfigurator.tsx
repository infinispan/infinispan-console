...
const TracingCacheConfigurator = (props: { tracingEnabled: boolean }) => {
  ...
  const [tracingConf, setTracingConf] = useState(
    configuration.advanced.tracing,
  );
  ...
  const validateForm = (): 'success' | 'default' | 'error' => {
    return tracingConf.enabled && tracingConf.categories.length == 0
      ? 'error'
      : 'success';
  };
  ...
  const tracingFeatureValidation = (): boolean => {
    return (
      !tracingConf.globalEnabled ||
      !tracingConf.enabled ||
      (tracingConf.enabled && tracingConf.categories.length > 0)
    );
  };
  ...
  const onSelectCategories = (selection) => {
    if (tracingConf.categories.includes(selection)) {
      setTracingConf((prevState) => {
        return {
          ...prevState,
          categories: tracingConf.categories.filter(
      ...
      <FormGroup fieldId="form-tracing">
        <Switch
          ...
          label={
            tracingConf.enabled
              ? t(
                  'caches.create.configurations.advanced-options.tracing-disable',
                )
              : t(
                  'caches.create.configurations.advanced-options.tracing-enable',
                )
          }
        />
        <PopoverHelp
          ...
          content={t(
            'caches.create.configurations.advanced-options.tracing-tooltip',
          )}
        />
      </FormGroup>
    );
  ...
  const buildContent = () => {
    ...
    return (
      <FormSection
        title={t('caches.create.configurations.advanced-options.tracing-title')}
      >
        {enableTracing()}
        <FormGroup fieldId="select-categories" isRequired label={'Categories'}>
          <SelectMultiWithChips
            id="categorySelector"
            placeholder={t(
              'caches.create.configurations.advanced-options.select-tracing-categories',
            )}
          ...
          {validateForm() === 'error' && (
            <FormHelperText>
              <HelperText>
                ...
                >
                  {t(
                    'caches.create.configurations.advanced-options.select-tracing-categories-helper',
                  )}
                </HelperTextItem>
              </HelperText>
            </FormHelperText>
