...
const BackupsCacheConfigurator = (props: {
    ...
    const formBackupSites = () => {
        return (
            <FormGroup
                ...
                labelIcon={<PopoverHelp name="backup-sites" label={t('caches.create.configurations.feature.backups-site')} content={t('caches.create.configurations.feature.backups-site-tooltip', { brandname: brandname })} />}
            >
                <Select
      ...
      return (
        <React.Fragment>
          ...
          {sites.map((site, index) => {
            ...
            return (
              <FormGroup key={'strategy-' + site}
                         fieldId={'strategy-' + site}
                         isInline
                         label={site.siteName}
              >
                <Radio
                  name={index.toString()}
                  id={`async-${index}`}
                  onChange={() => { site.siteStrategy = BackupSiteStrategy.ASYNC, setSites(sites.map(chip => chip.siteName === site.siteName ? site : chip)) }}
                ...
                <Radio
                  name={index.toString()}
                  id={`sync-${index}`}
                  onChange={() => { site.siteStrategy = BackupSiteStrategy.SYNC, setSites(sites.map(chip => chip.siteName === site.siteName ? site : chip)) }}
    ...
    const formBackupFor = () => {
      ...
      return (
            <React.Fragment>
                <FormGroup fieldId='remote-cache'
                           isInline
                           isRequired
                           label={t('caches.create.configurations.feature.remote-cache')}
                           labelIcon={<PopoverHelp name="remote-cache" label={t('caches.create.configurations.feature.remote-cache')} content={t('caches.create.configurations.feature.remote-cache-tooltip', { brandname: brandname })} />}
                           validated={validateBackupsForField(remoteCache)}
                >
                    <TextInput validated={validateBackupsForField(remoteCache)}
                               value={remoteCache}
                               onChange={(val) => {
                ...
                <FormGroup fieldId='remote-site'
                           isInline
                           validated={validateBackupsForField(remoteSite)}
                           isRequired
                           labelIcon={<PopoverHelp name="remote-site" label={t('caches.create.configurations.feature.remote-site')} content={t('caches.create.configurations.feature.remote-site-tooltip')} />}
                >
                    <Select
                        validated={validateBackupsForField(remoteSite)}
  ...
  if (!props.isEnabled) {
    return (
      <FeatureAlert feature={CacheFeature.BACKUPS}/>
    )
  }
