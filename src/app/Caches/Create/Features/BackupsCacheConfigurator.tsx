...
const BackupsCacheConfigurator = (props: { isEnabled: boolean }) => {
  ...
  const [enableBackupFor, setEnableBackupFor] = useState(
    configuration.feature.backupsCache.backupFor.enabled,
  );
  const [remoteCache, setRemoteCache] = useState(
    configuration.feature.backupsCache.backupFor.remoteCache!,
  );
  const [remoteSite, setRemoteSite] = useState(
    configuration.feature.backupsCache.backupFor.remoteSite!,
  );

  const [availableSites, setAvailableSites] = useState<string[]>([]);
  const [sites, setSites] = useState<BackupSiteBasic[]>(
    configuration.feature.backupsCache.sites!,
  );
    ...
    return (
      <FormGroup
        ...
        labelHelp={
          <PopoverHelp
            ...
            label={t('caches.create.configurations.feature.backups-site')}
            content={t(
              'caches.create.configurations.feature.backups-site-tooltip',
              { brandname: brandname },
            )}
          />
        }
      >
        <SelectMultiWithChips
          ...
          create={true}
          selection={
            sites.some((s) => s.siteName !== undefined)
              ? sites.map((s) => s.siteName as string)
              : []
          }
        />
        {sites.length == 0 && (
      ...
      <React.Fragment>
        <HelperText>
          <HelperTextItem icon={<InfoIcon />}>
            {t('caches.create.configurations.feature.strategy')}
          </HelperTextItem>
          <HelperTextItem>
            {t('caches.create.configurations.feature.strategy-tooltip', {
              brandname: brandname,
            })}
          </HelperTextItem>
        </HelperText>
              ...
              <Radio
                ...
                onChange={() => {
                  ((site.siteStrategy = BackupSiteStrategy.ASYNC),
                    setSites(
                      sites.map((chip) =>
                        chip.siteName === site.siteName ? site : chip,
                      ),
                    ));
                }}
        ...
        <FormGroup
          ...
          labelHelp={
            <PopoverHelp
              ...
              label={t('caches.create.configurations.feature.remote-cache')}
              content={t(
                'caches.create.configurations.feature.remote-cache-tooltip',
                { brandname: brandname },
              )}
            />
          }
        ...
        <FormGroup
          ...
          labelHelp={
            <PopoverHelp
              ...
              label={t('caches.create.configurations.feature.remote-site')}
              content={t(
                'caches.create.configurations.feature.remote-site-tooltip',
              )}
            />
          }
        >
  ...
  return (
    <FeatureCard
      computedTitle={t('caches.create.configurations.feature.backups', {
        local_site_name: localSite,
      })}
    >
      {formBackupSites()}
      {backupsStrategy()}
      <FormGroup fieldId="enable-backupfor">
        ...
        <PopoverHelp
          ...
          label={t('caches.create.configurations.feature.backup-for')}
          content={t(
            'caches.create.configurations.feature.backup-for-tooltip',
            { brandname: brandname },
          )}
        />
      </FormGroup>
