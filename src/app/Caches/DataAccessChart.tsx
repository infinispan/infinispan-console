...
const DataAccessChart = (props: { stats: CacheStats }) => {
  ...
  return (
    <Card>
      <CardTitle>
        <PopoverHelp
          name={'pop-over-data-access-chart-help'}
      ...
      <CardBody>
        <div style={{ width: '100%', height: '480px' }}>
          <ChartDonut
            ...
            data={[
              {
                x: t('caches.cache-metrics.data-access-hits'),
                y: props.stats.hits,
              },
              {
                x: t('caches.cache-metrics.data-access-misses'),
                y: props.stats.misses,
              },
              {
                x: t('caches.cache-metrics.data-access-stores'),
                y: props.stats.stores,
              },
              {
                x: t('caches.cache-metrics.data-access-retrievals'),
                y: props.stats.retrievals,
              },
              {
                x: t('caches.cache-metrics.data-access-remove-hits'),
                y: props.stats.remove_hits,
              },
              {
                x: t('caches.cache-metrics.data-access-remove-misses'),
                y: props.stats.remove_misses,
              },
              {
                x: t('caches.cache-metrics.data-access-evictions'),
                y: props.stats.evictions,
              },
            ]}
            labels={({ datum }) =>
              `${datum.x}:${displayUtils.formatNumber((datum.y * 100) / all)}%`
            }
            legendData={[
              {
                name:
                  ...
                  ': ' +
                  displayUtils.formatNumber(props.stats.hits),
              },
              {
                name:
                  t('caches.cache-metrics.data-access-misses') +
                  ': ' +
                  displayUtils.formatNumber(props.stats.misses),
              },
              {
                name:
                  t('caches.cache-metrics.data-access-stores') +
                  ': ' +
                  displayUtils.formatNumber(props.stats.stores),
              },
              {
                name:
