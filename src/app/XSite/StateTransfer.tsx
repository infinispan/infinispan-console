...
const StateTransfer = (props: {
  ...
  const buildContent = () => {
    if(props.action == 'cancel') {
      return (
        <Text>
          ...
          cache  <strong>{props.cacheName}</strong> to the site <strong>{props.siteName}</strong>.
          <br/>
    ...
    return (
      <Text>
      The state transfer operation implies transferring the data from cache <strong>{props.cacheName}</strong> to
      its backup cache in the site <strong>{props.siteName}</strong>. This operation can take long.
      <br/>
