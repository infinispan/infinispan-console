...
const ClusterDisplay = (props: { cacheManager: undefined | CacheManager }) => {
  ...
  let size =
    cm.cluster_size > 1
      ? cm.cluster_size + ' members'
      : cm.cluster_size + ' member';
  return (
      ...
      <CardBody>
        <Grid>
          <GridItem span={2}>
            <Stack>
              ...
              <StackItem>
                <Label
                  style={{
                    backgroundColor: displayUtils.healthColor(cm.health)
                  }}
                >
                  {cm.health}
                </Label>
          ...
          <GridItem span={2}>
            <Stack>
              <StackItem>
                <h1 style={{ fontSize: 22 }}>{cm.cluster_name}</h1>
              </StackItem>
              ...
              {cm.cluster_members.map((mem, index) => (
                <GalleryItem>
                  <Stack>
                    ...
                    <StackItem>
                      {cm.cluster_members_physical_addresses[index]}
                    </StackItem>
                  </Stack>
                </GalleryItem>
              ))}
