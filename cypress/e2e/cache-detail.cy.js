...
describe('Cache Detail Overview', () => {
 ...
 it('successfully adds new entry', () => {
   ...
   cy.get('#key-entry').click().type('{"_type": "org.infinispan.Person","name": "Elaia","age" : 12}',{ parseSpecialCharSequences: false });
   cy.get('#value-entry').click().type('{"_type": "org.infinispan.Person","name": "Elaia","age" : 12}',{ parseSpecialCharSequences: false });
   cy.get('[data-cy=addButton]').click();
   cy.contains('Entry added to cache people.');
   cy.get('.pf-c-alert__action > .pf-c-button').click(); //Closing alert popup.
   ...
   cy.get('#keyContentType').click();
   cy.get('#double').click();
   cy.get('#key-entry').click().type(1.7976931348623157E+308);
   cy.get('#valueContentType').click();
   cy.get('#double').click();
   cy.get('#value-entry').click().type(1.7976931348623157E+308);
   cy.get('[data-cy=addButton]').click();
   cy.contains('Entry added to cache people.');
   cy.get('.pf-c-alert__action > .pf-c-button').click(); //Closing alert popup.
   ...
   cy.get('#keyContentType').click();
   cy.get('#float').click();
   cy.get('#key-entry').click().type(3.402823466E+38);
   cy.get('#valueContentType').click();
   cy.get('#float').click();
 ...
 it('successfully searches entries by key', () => {
  ...
  cy.get("#keyType").click();
  cy.get("[id='Custom Type']").click();
  cy.get("#textSearchByKey").click().type('{"_type": "org.infinispan.Person","name": "Elaia","age" : 12}',{ parseSpecialCharSequences: false });
  cy.get("[data-cy=search-by-key]").click();
  cy.contains("true").should("not.exist");
  cy.contains("Elaia");
  cy.contains("1 - 1 of 1");
